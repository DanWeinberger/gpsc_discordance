...
cog_freq <- unique.genes %>%
  mutate(change_st= if_else(stA!=stB,1,0)) %>%
    group_by(Gene, Annotation,change_st) %>%
    summarise('cog_freq'=n()) %>%
    arrange(desc(cog_freq)) %>%
    mutate(change_st= paste0('st_change_', change_st))

#this evaluates how often we see discordance in the locus if closest neighbor is a ST switch or not.

cog_freq.c <- reshape2::dcast(cog_freq, Gene+Annotation ~ change_st) %>%
  filter(st_change_1 + st_change_0 >=20) %>%
  mutate(ratio= (st_change_1/sum(all_switches2$change_st==1))/(st_change_0/sum(all_switches2$change_st==0))) %>%
  arrange(-ratio)
